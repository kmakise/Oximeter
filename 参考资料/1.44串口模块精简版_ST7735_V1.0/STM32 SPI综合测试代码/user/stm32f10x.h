...
typedef struct
{
  __IO uint32_t IMR;	  //中断屏蔽寄存器	   1为不被屏蔽 0相反  19位线
  __IO uint32_t EMR;	  //事件屏蔽寄存器	   1为不被屏蔽 0相反  19位线
  __IO uint32_t RTSR;	  //上升沿触发选择寄存器   1为允许 0相反  19位线
  __IO uint32_t FTSR;	  //下升沿触发选择寄存器   1为允许 0相反  19位线
...
typedef struct
{
  __IO uint32_t CRL;   //端口配置寄存器（低8位）
  __IO uint32_t CRH;   //端口配置寄存器（高8位）
  __IO uint32_t IDR;   //数据输入寄存器
  __IO uint32_t ODR;   //数据输出寄存器
  __IO uint32_t BSRR;  //端口置位复位寄存器
  __IO uint32_t BRR;   //端口复位寄存器
